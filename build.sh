...
if echo $BUILD_TARGETS | grep "aarch64"; then
    ...
    cp target/aarch64-linux-android/debug/libecho_jni.so library/src/main/jniLibs/arm64-v8a
fi
if echo $BUILD_TARGETS | grep "x86_64"; then
    ...
    cp target/x86_64-linux-android/debug/libecho_jni.so library/src/main/jniLibs/x86_64
fi
if echo $BUILD_TARGETS | grep "armv7"; then
    ...
    cp target/armv7-linux-androideabi/debug/libecho_jni.so library/src/main/jniLibs/armeabi-v7a
fi
if echo $BUILD_TARGETS | grep "i686"; then
    ...
    cp target/i686-linux-android/debug/libecho_jni.so library/src/main/jniLibs/x86
fi
